...
import { WithdrawRateUtil } from './dataLayer/withdrawRateUtil';
...
export const withdrawRateService = preflightChecking(
  async (request, returnResponse) => {
    ...
    try {
      if (request.method === 'POST') {
        ...
        const resultFromDb: WithdrawRate | void = await WithdrawRateUtil.getRateList().catch(
          error => {
